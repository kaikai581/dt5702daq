...
void FEBDTP::Init_FEBDTP_pkt(FEBDTP_PKT *pkt, unsigned char *src, unsigned char *dst)
{
...
int FEBDTP::Send_pkt(FEBDTP_PKT *pkt, int len, int timeout_us)
{
  ...
  while (numbytes >= 0)
  {
    ...
    {
      if (Verbose > 0)
        printf("****************  Packet received from wrong FEB: 0x%02x (must be 0x%02x) Skipping..\n", gpkt.src_mac[5], thisdstmac[5]);
      continue;
    } // NOT from our FEB
    ...
    if (gpkt.CMD == FEB_DATA_CDR || gpkt.CMD == FEB_DATA_FW)
    {
      ...
      if (fPacketHandler)
        fPacketHandler(numbytes); //Do something with received data
    }
    //else
    if (Verbose > 0)
      printf("<< ");
...
int FEBDTP::SendCMD(unsigned char *mac, unsigned short cmd, unsigned short reg, unsigned char *buf)
{
  int numbytes = 1;
  int retval = 0;
  int packlen = 64;
...
void FEBDTP::CMD_stoa(unsigned short cmd, char *str)
{
  switch (cmd)
  {
...
int FEBDTP::ReadBitStream(const char *fname, unsigned char *buf) // read CITIROC SC bitstream into the buffer, buf[MAXPACKLEN]
{
  ...
  while (fgets(line, sizeof(line), file))
  {
    ...
    while (bit != 0x27 && bit != 0 && ptr < sizeof(line) && bitlen < MAXPACKLEN) // ASCII(0x27)= '
    {
      bit = line[ptr];
      ptr++;
      if (bit == 0x20 || bit == 0x27)
        continue; //ignore spaces and apostrophe
      if (Verbose)
        printf("%c", bit);
      ascii[bitlen] = bit;
      bitlen++;
...
int FEBDTP::WriteBitStream(const char *fname, unsigned char *buf, int bitlen) // write CITIROC SC bitstream to file, buf[MAXPACKLEN]
{
  ...
  for (int i = 0; i < bitlen; i++)
    fprintf(file, "%c", ascii[i]);
  fprintf(file, "\n");
  fclose(file);
  return bitlen;
}

int FEBDTP::WriteBitStreamAnnotated(const char *fname, unsigned char *buf, int bitlen) // write CITIROC SC bitstream to file, buf[MAXPACKLEN]
{
  FILE *file = fopen(fname, "w");
  unsigned char byte;
  int ib = 0;
  ...
  for (int Ch = 0; Ch < 32; Ch++)
  {
    fprintf(file, "%c%c%c", ascii[ib], ascii[ib + 1], ascii[ib + 2]);
    ib += 3;
    fprintf(file, "%c%c%c ", ascii[ib], ascii[ib + 1], ascii[ib + 2]);
    ib += 3;
    fprintf(file, "%c%c%c", ascii[ib], ascii[ib + 1], ascii[ib + 2]);
...
void FEBDTP::WriteLVBitStream(const char *fname, unsigned char *buf, bool rev) // write CITIROC SC bitstream from the buffer, buf[MAXPACKLEN], to LabView setup file
{
  int i = 0;
  FILE *file = fopen(fname, "w");
  //   fprintf(file,"Normal:\n");
...
void FEBDTP::ReadLVBitStream(const char *fname, unsigned char *buf, bool rev)
{
  int i = 0;
  unsigned char bt;
  char line[1024];
